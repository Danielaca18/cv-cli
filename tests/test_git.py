...
from src.cv_cli.git import get_repo_url, git_init, git_clone, git_pull, git_push, git_sync
...
def test_get_repo_url(monkeypatch):
    ...
    monkeypatch.setattr("src.cv_cli.git.run", mock)
...
def test_git_init(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("src.cv_cli.git.run", mock)
    monkeypatch.setattr("src.cv_cli.git.get_repo_url", mock_get_url)
    monkeypatch.setattr("src.cv_cli.git.git_sync", MagicMock())
...
def test_git_init_no_repo(monkeypatch, tmp_path):
    mock_run = MagicMock()
    monkeypatch.setattr("src.cv_cli.git.run", mock_run)
    monkeypatch.setattr("src.cv_cli.git.get_repo_url", MagicMock(side_effect=FileNotFoundError))
    monkeypatch.setattr("src.cv_cli.git.git_sync", MagicMock())
...
def test_git_clone(monkeypatch, tmp_path):
    ...
    mock_rm = MagicMock()
    monkeypatch.setattr("src.cv_cli.git.run", mock)
    monkeypatch.setattr("src.cv_cli.git.rmtree", mock_rm)
...
def test_git_sync(monkeypatch, tmp_path):
    mock_push = MagicMock()
    mock_pull = MagicMock()
    monkeypatch.setattr("src.cv_cli.git.git_push", mock_push)
    monkeypatch.setattr("src.cv_cli.git.git_pull", mock_pull)
